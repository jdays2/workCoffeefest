...
//открыть большую отдельную модалку с картой
const mainMapBtn = document.querySelector('.map-btn__wrapper');
const mainMap = document.querySelector('.map-modal');

if (mainMapBtn && mainMap) {
	const activeClass = 'active';
	mainMapBtn.addEventListener('click', () => {
		mainMap.classList.add(activeClass);
	});
}
